...
const force_flag = zli.Flag{
    .name = "force",
    .shortcut = "f",
    .description = "Kill the instance if it is still running, then delete",
    .type = .Bool,
...
pub fn register(init_options: zli.InitOptions) !*zli.Command {
    const cmd = try zli.Command.init(init_options, .{
        .name = "delete",
        .description = "Permanently remove an instance: its registration and all its data",
    }, deleteCmd);
...
fn deleteCmd(ctx: zli.CommandContext) !void {
    ...
    const data_dir = try std.fs.path.join(allocator, &.{ root, ".lws", inst.service, inst.name });
    defer allocator.free(data_dir);
    std.Io.Dir.cwd().deleteTree(io, data_dir) catch |err| {
        try out.print("removed registry but failed to delete data dir {s}: {s}\n", .{ data_dir, @errorName(err) });
        return;
    };

    try out.print("deleted {s} instance '{s}' and all its data\n", .{ inst.service, inst.name });
}
