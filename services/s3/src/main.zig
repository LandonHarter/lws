...
const bucket_config = @import("bucket_config.zig");
...
pub fn main(init: std.process.Init) !void {
    ...
    while (args.next()) |arg| {
        if (std.mem.eql(u8, arg, "--port")) {
            ...
            if (std.mem.eql(u8, val, "off") or std.mem.eql(u8, val, "false")) {
                ...
                std.debug.print("s3: invalid --fsync value '{s}' (want on|off)\n", .{val});
                return error.InvalidFsyncValue;
            }
        } else {
            std.debug.print("s3: unknown arg '{s}'\n", .{arg});
    ...
    if (cfg.generate_config) {
        ...
        try bucket_config.writeDefaults(stdout);
        try stdout.flush();
        return;
    }

    if (log.Level.parse(cfg.log_level) == null) {
        std.debug.print("s3: invalid --log-level '{s}' (want error|warn|info|debug)\n", .{cfg.log_level});
    ...
    std.debug.print("s3 listening on {s}:{d} (host '{s}', account {s}, region {s}), data-dir '{s}'\n", .{ cfg.bind, cfg.port, cfg.host, cfg.account_id, cfg.region, cfg.data_dir });
